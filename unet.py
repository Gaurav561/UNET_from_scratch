...
class UNET(self,shape):
    def __init__(self,shape):
        self.shape = shape

        def encoder_block(self,input_to_conv,filters):
            c = Conv2D(filters,(3,3),activation = "relu")(input_to_conv)
            x = Conv2D(filters,(3,3),activation = "relu")(c)
            m = MaxPool2D((2,2))(x)
        ...
        def decoder_block(self,input_to_upconv,conc_layer,filters):
        ...
        def model(self):
            input_layer  = tf.keras.layers.Input(self.shape)
